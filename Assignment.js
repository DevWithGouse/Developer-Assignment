// For given Developer Assignment, The solution has been implemented using JavaScript.

// Logic to generate the reports
function processSalesData(data) {
    const lines = data.trim().split('\n'); 
    let monthlySales = {};
    let itemPopularity = {};
    let itemRevenue = {};
    let mostPopularItemStats = {}; 
    ...
    lines.forEach((line) => {
        ...
        if (!itemPopularity[month]) itemPopularity[month] = {};
        if (!itemPopularity[month][sku]) itemPopularity[month][sku] = 0;
        itemPopularity[month][sku] += quantity;

        if (!itemRevenue[month]) itemRevenue[month] = {};
        if (!itemRevenue[month][sku]) itemRevenue[month][sku] = 0;
        itemRevenue[month][sku] += totalPrice;
    });

    generateReports(monthlySales, itemPopularity, itemRevenue, mostPopularItemStats);
}


function generateReports(monthlySales, itemPopularity, itemRevenue, mostPopularItemStats) {
    const totalSales = Object.values(monthlySales).reduce((acc, val) => acc + val, 0);
    console.log('Total Sales of the Store: ', totalSales);
    console.log('Month-wise Sales Totals:', monthlySales);

    Object.keys(itemPopularity).forEach((month) => {
        const popularItem = Object.keys(itemPopularity[month]).reduce((a, b) =>
            itemPopularity[month][a] > itemPopularity[month][b] ? a : b
        );
        const revenueItem = Object.keys(itemRevenue[month]).reduce((a, b) =>
            itemRevenue[month][a] > itemRevenue[month][b] ? a : b
        );

        console.log(`Most popular item in month ${month}: ${popularItem}`);
        console.log(`Item generating most revenue in month ${month}: ${revenueItem}`);

        if (!mostPopularItemStats[popularItem]) mostPopularItemStats[popularItem] = [];
        mostPopularItemStats[popularItem].push(itemPopularity[month][popularItem]);
    });

    Object.keys(mostPopularItemStats).forEach((item) => {
        const quantities = mostPopularItemStats[item];
        ...
        console.log(`For the most popular item: "${item}", Min Orders: ${minOrders}, Max Orders: ${maxOrders}, Average Orders: ${avgOrders.toFixed(2)}`);
    });
}



// Hardcoded sales data or we can use axios to get data from "https://insurecomp.com/sales-data.txt" 
const salesData = `
...
// Calling function using sales data
